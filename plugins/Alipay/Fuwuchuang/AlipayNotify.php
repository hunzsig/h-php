...
class AlipayNotify extends AbstractLib{
	...
	public function verifyNotify(){
		if(empty($_POST)) {//判断POST来的数组是否为空
			...
			if ($this->alipay_config['sign_type'] == '0001') {
				$decrypt_post_para['notify_data'] = $this->getRsa()->rsaDecrypt($decrypt_post_para['notify_data'], $this->alipay_config['private_key_path']);
			}
			...
			$responseTxt = 'true';
			if (! empty($notify_id)) {$responseTxt = $this->getRsaponse($notify_id);}
	...
	public function decrypt($prestr) {
		return $this->getRsa()->rsaDecrypt($prestr, trim($this->alipay_config['private_key_path']));
	}
	...
	public function getSignVeryfy($para_temp, $sign, $isSort) {
		//除去待签名参数数组中的空值和签名参数
		$para = $this->getCore()->paraFilter($para_temp);

		//对待签名参数数组排序
		if($isSort) {
			$para = $this->getCore()->argSort($para);
		} else {
			$para = $this->sortNotifyPara($para);
		}

		//把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
		$prestr = $this->getCore()->createLinkstring($para);

		$isSgin = false;
		switch (strtoupper(trim($this->alipay_config['sign_type']))) {
			case "MD5" :
				$isSgin = $this->getMd5()->md5Verify($prestr, $sign, $this->alipay_config['key']);
				break;
			case "RSA" :
				$isSgin = $this->getRsa()->rsaVerify($prestr, trim($this->alipay_config['ali_public_key_path']), $sign);
				break;
			case "0001" :
				$isSgin = $this->getRsa()->rsaVerify($prestr, trim($this->alipay_config['ali_public_key_path']), $sign);
				break;
	 ...
	 */
	public function getRsaponse($notify_id) {
