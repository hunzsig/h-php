...
class AlipaySubmit extends AbstractLib{
	...
	public function buildRequestHttpInFile($para_temp, $file_para_name, $file_name) {
		...
		//远程获取数据
		$sResult = $this->getCore()->getHttpResponsePOST($this->alipay_gateway_new, $this->alipay_config['cacert'],$para,trim(strtolower($this->alipay_config['input_charset'])));
	...
	public function parseResponse($str_text) {
		...
		if( ! empty ($para_text['res_data'])) {
			...
			if($this->alipay_config['sign_type'] == '0001') {
				$para_text['res_data'] = $this->getRsa()->rsaDecrypt($para_text['res_data'], $this->alipay_config['private_key_path']);
			}
